import React from 'react'
import {Tab, Row, Col, ListGroup} from 'react-bootstrap'
import avatar from '../image/pain.jpg'
import Reviews from './Reviews'
const Profileuser = () =>{
    return(
        <>
        <div className="display">
            <div className="profile-image">
                <img className="rounded-image" alt="profile-pic" src="data:image/png;base64," />
 
            </div>
             <div className="profile-name">
    <h3>Marvellous filani</h3>
    <p>Boston, Massachusetts</p>
    </div>
    <div className="">
    <p> Rating </p>
    </div>
  
        </div>
        <Tab.Container id="list-group-tabs-example" defaultActiveKey="#link1">
  <Row className="profile-margin margin-row">
    <Col sm={12} md={4} xl={4} className="col-color">
      <ListGroup className="grop">
        <ListGroup.Item className="change" action href="#link1">
         <img src={avatar} className="avatar-icon" alt="avatar"/> Profile Overview
        </ListGroup.Item>
        <ListGroup.Item className="change" action href="#link2">
        Reviews
        </ListGroup.Item>
        <ListGroup.Item className="change" action href="#link3">
          Link 3
        </ListGroup.Item>
        <ListGroup.Item className="change" action href="#">
          Link 2
        </ListGroup.Item>
      </ListGroup>
    </Col>
    <Col sm={12} md={7} xl={7} className="col-8-color">
      <Tab.Content className="review-content">
        <Tab.Pane eventKey="#link1">
          <p>This is one</p>
        </Tab.Pane>
        <Tab.Pane eventKey="#link2">
        <div className="card-review">
            <h3 className="my-color">Reviews</h3>
            
                <label> by Rating 
            <select >
    <option value="Date">Date</option>
    <option value="Rating">Rating</option>
    <option value="C">Cranberry</option>
  </select>
  </label>
  </div>
  <Reviews/>    
          </Tab.Pane>
          <Tab.Pane eventKey="#link3">

          </Tab.Pane>
      </Tab.Content>
    </Col>
  </Row>
</Tab.Container>

        </>
    )
}
export default Profileuser